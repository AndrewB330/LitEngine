...
std::optional<Collision> CollisionEngine::GetCollision(int a_id, int b_id) const {
    ...
    if (glm::length2(v[0].v) < 1e-6) v[0].v = Vec3(0.00001, 0, 0);

    Vec3 n = -v[0].v;
    v[1] = support(a, b, n);
    if (glm::dot(v[1].v, n) <= 0.0) {
        return std::nullopt;
    }

    n = glm::cross(v[1].v,v[0].v);

    if (glm::length2(v[0].v) < 1e-6) {
        Collision collision;
        collision.a_id = a_id;
        collision.b_id = b_id;
        collision.point_a = v[1].a;
        collision.point_b = v[1].b;
        collision.normal = glm::normalize(v[1].b - v[0].a);
        collision.penetration = glm::length(v[1].b - v[0].a);
        return collision;
    }

    v[2] = support(a, b, n);
    if (glm::dot(v[2].v, n) <= 0.0) {
        return std::nullopt;
    }

    n = glm::cross(v[1].v - v[0].v,v[2].v - v[0].v);
    double dist = glm::dot(n, v[0].v);
    ...
    while (!hit) {
        v[3] = support(a, b, n);
        if (glm::dot(v[3].v, n) <= 0.0f) {
            return std::nullopt;
        }

        if (glm::dot(glm::cross(v[1].v,v[3].v), v[0].v) < 0.0f) {
            v[2] = v[3];
            n = glm::cross(v[1].v - v[0].v, v[2].v - v[0].v);
            continue;
        }

        if (glm::dot(glm::cross(v[3].v, v[2].v), v[0].v) < 0.0f) {
            v[1] = v[3];
            n = glm::cross(v[1].v - v[0].v, v[2].v - v[0].v);
            continue;
        }
        ...
        while (1) {
            ...
            n = glm::normalize(glm::cross(v[2].v - v[1].v, v[3].v - v[1].v));
            double d = glm::dot(n,v[1].v);

            if (d >= 0)
                hit = true;

            v[4] = support(a, b, n);
            double delta = glm::dot(v[4].v - v[3].v, n);
            double depth = -glm::dot(n, v[4].v);
            ...
            if (delta <= 0.001f || depth >= 0 || pass > 30) {
                ...
                double bar[4];
                bar[0] = glm::dot(glm::cross(v[1].v, v[2].v), v[3].v);
                bar[1] = glm::dot(glm::cross(v[3].v, v[2].v), v[0].v);
                bar[2] = glm::dot(glm::cross(v[0].v, v[1].v), v[3].v);
                bar[3] = glm::dot(glm::cross(v[2].v, v[1].v), v[0].v);

                double sum = bar[0] + bar[1] + bar[2] + bar[3];

                if (sum <= 0) {
                    bar[0] = 0.0f;
                    bar[1] = glm::dot(glm::cross(v[2].v, v[3].v), n);
                    bar[2] = glm::dot(glm::cross(v[3].v, v[1].v), n);
                    bar[3] = glm::dot(glm::cross(v[1].v, v[2].v), n);
            ...
            double d1 = glm::dot(glm::cross(v[4].v, v[1].v), v[0].v);
            double d2 = glm::dot(glm::cross(v[4].v, v[2].v), v[0].v);
            double d3 = glm::dot(glm::cross(v[4].v, v[3].v), v[0].v);
    ...
    collision.a_id = a_id;
    collision.b_id = b_id;
    collision.point_a = n * glm::dot(s.a - p1, n) + p1;
    collision.point_b = n * glm::dot(s.b - p2, n) + p2;
    collision.normal = glm::normalize(n);
    collision.penetration = glm::dot(collision.normal, v[4].v);
    return collision;
}
