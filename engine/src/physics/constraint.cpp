...
ContactConstraint::ContactConstraint(PhysObjectPtr a, PhysObjectPtr b, ContactPoint point)
    ...
    initial_point_a = a->GetTranslation() + glm::rotate(a->GetRotation(), local_lever_a);
    initial_point_b = b->GetTranslation() + glm::rotate(b->GetRotation(), local_lever_b);
}
...
void ContactConstraint::Init() {
    ...
    lever_a = glm::rotate(a->GetRotation(), local_lever_a);
    lever_b = glm::rotate(b->GetRotation(), local_lever_b);
    point_a = a->GetTranslation() + lever_a;
    point_b = b->GetTranslation() + lever_b;
    penetration = glm::dot(point_b - point_a, normal);

    if (glm::length2(normal_p) > 1e-3) {
        normal_q = glm::normalize(glm::cross(normal, normal_p));
        normal_p = glm::normalize(glm::cross(normal_q, normal));
    } else {
        Vec3 random_vec = RandomUnit();
        while (fabs(glm::length2(glm::cross(random_vec, normal))) < 1e-4) {
            random_vec = RandomUnit();
        }
        normal_p = glm::normalize(glm::cross(normal, random_vec));
        normal_q = glm::normalize(glm::cross(normal, normal_p));
    }
    ...
    ENSURE_THAT_BETWEEN(glm::dot(normal, normal_q), -1e-3, +1e-3)
    ENSURE_THAT_BETWEEN(glm::dot(normal, normal_p), -1e-3, +1e-3)
    ENSURE_THAT_BETWEEN(glm::dot(normal_p, normal_q), -1e-3, +1e-3)*/
...
double ContactConstraint::GetEffectiveMass(const Vec3 &norm) const {
    return 1.0 / (
            ...
            glm::dot(glm::cross(a->GetInertia().moment_of_inertia_inv_global * glm::cross(lever_a, norm), lever_a), norm) +
            glm::dot(glm::cross(b->GetInertia().moment_of_inertia_inv_global * glm::cross(lever_b, norm), lever_b), norm)
    );
}
...
bool ContactConstraint::IsResolved() const {
    ...
    return penetration < -1e-2 ||
           glm::distance(initial_point_a, point_a) > 0.05 ||
           glm::distance(initial_point_b, point_b) > 0.05 ||
           glm::distance(point_a, point_b) > fabs(4.0 * penetration) + ALLOWED_PENETRATION;
}
...
void ContactConstraint::Solve(int phase) {
    ...
    Vec3 point_velocity_a = a->GetAccumulatedVelocity() + glm::cross(a->GetAccumulatedAngularVelocity(), lever_a);
    Vec3 point_velocity_b = b->GetAccumulatedVelocity() + glm::cross(b->GetAccumulatedAngularVelocity(), lever_b);
    Vec3 rel_velocity = point_velocity_b - point_velocity_a;

    double lambda = glm::dot(rel_velocity, normal) * effective_mass;
    ...
    a->AddImpulse(+lambda * normal, +lambda * glm::cross(lever_a, normal));
    b->AddImpulse(-lambda * normal, -lambda * glm::cross(lever_b, normal));

    // Friction
    // todo: make friction_lambda to be Vec2 (not separate variables)
    double lambda_p = glm::dot(rel_velocity, normal_p) * effective_mass_p;
    double lambda_q = glm::dot(rel_velocity, normal_q) * effective_mass_q;
    ...
    double cur_friction_len = glm::length(cur_friction);
    ...
    a->AddImpulse(+lambda_p * normal_p, +lambda_p * glm::cross(lever_a, normal_p));
    b->AddImpulse(-lambda_p * normal_p, -lambda_p * glm::cross(lever_b, normal_p));

    a->AddImpulse(+lambda_q * normal_q, +lambda_q * glm::cross(lever_a, normal_q));
    b->AddImpulse(-lambda_q * normal_q, -lambda_q * glm::cross(lever_b, normal_q));
}
...
void ContactConstraint::SolvePseudoPhase() {
    ...
    double lambda_pseudo = (glm::dot(rel_velocity, normal) + (penetration - ALLOWED_PENETRATION) * 2.0) * effective_mass;
    ...
    a->AddPseudoImpulse(+lambda_pseudo * normal, +lambda_pseudo * glm::cross(lever_a, normal));
    b->AddPseudoImpulse(-lambda_pseudo * normal, -lambda_pseudo * glm::cross(lever_b, normal));
}
...
bool ContactConstraint::UpdateIfTheSame(ContactPoint contact) {
    ...
    if (glm::distance(local_lever_a, contact.local_lever_a) > CONTACT_RADIUS ||
        glm::distance(local_lever_b, contact.local_lever_b) > CONTACT_RADIUS ||
        glm::dot(normal, contact.normal) < NORMAL_THRESHOLD) {
        return false;
    }
    /**
