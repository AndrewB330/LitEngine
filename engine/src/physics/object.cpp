...
void PhysObject::ApplyChanges(double dt) {
    if (glm::length(GetAccumulatedVelocity()) < 0.01 && glm::length(GetAccumulatedAngularVelocity()) < 0.01) {
        no_motion_ticks++;
    } else {
        no_motion_ticks = 0;
    }
    ...
    auto v = GetAccumulatedAngularVelocity();
    SetRotation(GetRotation() + (0.5 * glm::dquat(0, v.x, v.y, v.z) * GetRotation()) * dt);
...
void PhysObject::RecalculateInertia() {
    if (fixed) {
        ...
        const Mat3 &rot = glm::toMat4(collider->rotation);
        inertia.moment_of_inertia_inv_global = rot * inertia.moment_of_inertia_inv * glm::transpose(rot);
    }
}
...
void PhysObject::Rotate(const Quat &rotation) {
    collider->rotation = glm::normalize(rotation * collider->rotation);
    Mat3 rot = glm::toMat4(GetRotation());
    inertia.moment_of_inertia_inv_global = rot * inertia.moment_of_inertia_inv * glm::transpose(rot);
}
...
void PhysObject::SetTranslation(const Vec3 &translate) {
    collider->translation = translate;
}
