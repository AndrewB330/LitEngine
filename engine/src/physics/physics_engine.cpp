...
void PhysicsEngine::Integrate(double dt) {
    ...
    for (auto &[id, o] : objects) {
        ...
        if (glm::length(o->GetTranslation()) > 10000) {
...
void PhysicsEngine::GenerateContactPoints() {
    for (auto collision : collision_engine.GetAllCollisions()) {
        ...
        point.local_lever_a = glm::rotate(glm::inverse(a->GetRotation()), collision.point_a - a->GetTranslation());
        point.local_lever_b = glm::rotate(glm::inverse(b->GetRotation()), collision.point_b - b->GetTranslation());
