...
namespace lit::voxels::shaders {
    ...
    shader_frag(LightingFrag) {
        shader_out(location = 0, vec4, out_light, gl::Attachment::RGB16);
        ...
        float try_sample_ambient(int i) {
            ivec2 coord = ivec2(gl_FragCoord.xy());

            vec3 normal = texelFetch(uni_normal, coord, 0).xyz();
            vec3 direction = normalize(normal * 0.1f + getRandomDirectionByNormal(uni_seed * 2 + i, normal));
            vec3 origin = texelFetch(uni_position, coord, 0).xyz() + normal * 1e-3;

            CastResult mainCast = RayCast(shadowObject, origin + direction, direction, 64);

            return 1.0f - float(mainCast.hit);
        }

        float try_sample_directional(int i) {
            ivec2 coord = ivec2(gl_FragCoord.xy());

            vec3 normal = texelFetch(uni_normal, coord, 0).xyz();
            vec3 direction = getRandomDirection(uni_seed * 2 + i) * uni_radius + uni_light_pos;
            ...
            return (1.0f - float(mainCast.hit)) * dot(direction, normal);
        }
        ...
        void main() {
            ...
            float light = 0;
            if (uni_light_type == 0) {
                light = try_sample_ambient(0) + try_sample_ambient(1);
            } else {
                light = try_sample_directional(0) + try_sample_directional(1);
            }
            out_light = vec4(uni_light_color * (light/2.0f), 1);
        }
    };
